...
```
{App}.sln
├── {App}.Domain/              ← Models (Rich Domain) + Services + Helpers (no circular refs)
...
DTO (no deps)    Infra.Interfaces (refs Domain + DTO)
      ↑                ↑
      ├── Domain ──────┘ (no ref to Infra.Interfaces — avoids circular dep)
      ├── Infra ─────────  (refs Domain + DTO + Infra.Interfaces)
      ├── Application ───  (refs Domain + Infra.Interfaces) → DI composition root for Domain Services
      └── MAUI ──────────  (refs all including Application) → Tests
```

**Note:** Domain does NOT reference Infra.Interfaces to avoid circular dependency (Infra.Interfaces already refs Domain). Domain Services that need repository interfaces should be registered via the Application project which refs both.
